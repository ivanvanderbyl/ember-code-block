...
module.exports = {
	...
	included(app) {
		...
		files.forEach(function(file) {
			target.import(`node_modules/highlight.js/lib/languages/${file.name}`, {
				using: [{ transformation: 'cjs', as: `highlight/${file.as}` }],
			})
		})
	...
	treeForVendor() {
		...
		define(['require', 'highlight'] , function (require, hljs) {
			return function () {
				${this.languages
					.map(lang => `hljs.registerLanguage('${lang}', require('highlight/${lang}').default);`)
					.join('\n')}
			};
		});
		`
	...
	_languageFiles(languages) {
		return languages.map(language => {
			...
			return { pathToLanguage, dir, name, as: language }
		})
	},
