...
module.exports = {
  ...
  included(app) {
    ...
    let files = this._languageFiles(this.languages)

    // 2. Import each language specified in config
    files.forEach(function(file) {
      target.import(`node_modules/highlight.js/lib/languages/${file.name}`, {
        using: [{ transformation: 'cjs', as: `highlight/${file.name}` }],
      })
    })
  ...
  treeForVendor() {
    let content = `
		define(['require', 'highlight'] , function (require, hljs) {
          ...
          .map(
            lang =>
              `hljs.registerLanguage('${lang}', require('highlight/${lang}'));`,
          )
          .join('\n')}
			};
		});
    ...
    let trees = [
      this.highlightTree(),
      // this.languagesTree(this.languages),
      registerFile,
    ]
  ...
  treeForStyles() {
    try {
      let pathToStyle = resolveSync(
        'highlight.js/styles/' + this.style + '.css',
      )
  ...
  _languageFiles(languages) {
    return languages.map(language => {
      let pathToLanguage = resolveSync(
  ...
  languagesTree(languages) {
    let files = this._languageFiles(languages)

    if (files.length === 0) {
      this.ui.writeWarnLine(
        '[ember-code-block] No languages configured to load',
      )
      return []
    }
